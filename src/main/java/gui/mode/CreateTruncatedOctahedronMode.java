...
import builders.TruncatedOctahedronBuilder;
...
/**
 * Create mode of trunctaedOctahedron by two points and angle
 *
 * @author kaznin
 */
public class CreateTruncatedOctahedronMode extends CreateBodyBy2PointsMode {

  public CreateTruncatedOctahedronMode(EdtController ctrl) {
    super(ctrl);
  }
  ...
  protected void setCreationMessengers() {
    _msg.setMessage("Выберите первую вершину усеченного октаэдра{MOUSELEFT}",
            "Выберите вторую вершину усеченного октаэдра{MOUSELEFT}",
            "Выберите положение{UP}{DOWN}{MOUSEWHEEL}{ENTER}");
  }

  @Override
  protected void setName() {
    _name = BodyType.TRUNCATED_OCTAHEDRON.getName(_ctrl.getEditor());
  }

  @Override
  public Icon getSmallIcon() {
    return IconList.TRUNCATED_OCTAHEDRON.getLargeIcon();
  }
  ...
  public String description() {
    return "<html><strong>Усеченный октаэдр</strong><br>по двум вершинам основания";
  }

  @Override
  public ModeList alias() {
    return ModeList.MODE_CREATE_TRUNCATEDOCTAHEDRON;
  }
  ...
  protected void create() {
    ...
    String[] defaultPointTitles = new String[24];
    String[] orderedPointTitles1 = new String[24];
    String[] orderedPointTitles2 = new String[24];
    ...
    boolean[] fixTitles = new boolean[24];
    fixTitles[0] = true;
    fixTitles[1] = true;
    for (int i = 2; i < 24; i++) {
      fixTitles[i] = false;
    ...
    TruncatedOctahedronBuilder builder = new TruncatedOctahedronBuilder(_name);
    ...
    if (cpnd.accepted) {
      if (cpnd.getChoice() != 0) {
        ...
        try {
          ...
          _ctrl.renameAnchorPoint(bd.getAnchorID("U"), values[20]);
          _ctrl.renameAnchorPoint(bd.getAnchorID("V"), values[21]);
          _ctrl.renameAnchorPoint(bd.getAnchorID("W"), values[22]);
          _ctrl.renameAnchorPoint(bd.getAnchorID("X"), values[23]);
          _ctrl.setUndo("Переименование точек усечённого октаэдра");
        } catch (ExNoBody ex) {
          if (LOG_LEVEL.value() >= 1) {
            Log.out.printf("Cannot rename truncated octahedron points: %s", ex.getMessage());
          }
        }
  ...
  protected void nativeDraw1(Render ren) {
    if (_numOfChosenAnchors == 2) {
      ...
      try {
        TruncatedOctahedron3d oct = new TruncatedOctahedron3d(_anchor.get(0).getPoint(),
                _anchor.get(1).getPoint(), valueAsDouble(ROT_ANGLE));
        Drawer.drawTruncatedOctahedron(ren, oct, TypeFigure.WIRE);
      } catch (ExGeom ex) {
      }
