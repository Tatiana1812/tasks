...
import builders.StellaOctahedronBuilder;
...
/**
 * Create mode of stellar octahedron by two points and angle
 *
 * @author kaznin
 */
public class CreateStellaOctahedronMode extends CreateBodyBy2PointsMode {

  public CreateStellaOctahedronMode(EdtController ctrl) {
    super(ctrl);
  }
  ...
  protected void setCreationMessengers() {
    _msg.setMessage("Выберите первую вершину звёздчатого октаэдра{MOUSELEFT}",
            "Выберите вторую вершину звёздчатого октаэдра{MOUSELEFT}",
            "Выберите положение{UP}{DOWN}{MOUSEWHEEL}{ENTER}");
  }

  @Override
  protected void setName() {
    _name = BodyType.STELLAR_OCTAHEDRON.getName(_ctrl.getEditor());
  }

  @Override
  public Icon getSmallIcon() {
    return IconList.STELLAR_OCTAHEDRON.getLargeIcon();
  }
  ...
  public String description() {
    return "<html><strong>Звёздчатый октаэдр</strong><br>по двум вершинам основания";
  }

  @Override
  public ModeList alias() {
    return ModeList.MODE_CREATE_STELLAOCTAHEDRON;
  }
  ...
  protected void create() {
    ...
    StellaOctahedronBuilder builder = new StellaOctahedronBuilder(_name);
    ...
    if (cpnd.accepted) {
      if (cpnd.getChoice() != 0) {
        ...
        try {
          ...
          _ctrl.setUndo("Переименование точек звёздчатого октаэдра");
        } catch (ExNoBody ex) {
          if (LOG_LEVEL.value() >= 1) {
            Log.out.printf("Cannot rename stellar octahedron points: %s", ex.getMessage());
          }
        }
  ...
  protected void doIfTwoPointsChosen(KeyEvent e) {
    ...
    if (isChosen(ROT_ANGLE)) {
      create();// создаём stella octahedron
    }
  }
  ...
  protected void nativeDraw1(Render ren) {
    if (_numOfChosenAnchors == 2) {
      ...
      try {
        StellaOctahedron3d so = new StellaOctahedron3d(_anchor.get(0).getPoint(),
                _anchor.get(1).getPoint(), valueAsDouble(ROT_ANGLE));
        Drawer.drawStellaOctahedron(ren, so, TypeFigure.WIRE);
      } catch (ExGeom ex) {
      }
