...
import builders.HyperboloidOfOneSheetBuilder;
 ...
 * @author Vitaliy
 */
public class CreateHyperboloidOfOneSheetMode extends CreateBodyBy3PointsMode {

  public CreateHyperboloidOfOneSheetMode(EdtController ctrl) {
    super(ctrl);
  }
  ...
  protected void setName() {
    _name = BodyType.HYPERBOLOID_OF_ONE_SHEET.getName(_ctrl.getEditor());
  }
  ...
  protected void nativeDraw1(Render ren) {
    ...
    if (_numOfChosenAnchors == 2 && !is3d) {
      try {
        HyperboloidOfOneSheet3d hyp = new HyperboloidOfOneSheet3d(anchor(0).getPoint(), anchor(1).getPoint(), currPoint);
        Drawer.drawHyperboloidOfOneSheet(ren, hyp, TypeFigure.CURVE);
      } catch (ExDegeneration ex) {
      }
    }
  }
  ...
  public ModeList alias() {
    return ModeList.MODE_CREATE_HYPERBOLOID_OF_ONE_SHEET;
  }

  @Override
  public Icon getSmallIcon() {
    return IconList.HYPERBOLOID_OF_ONE_SHEET.getLargeIcon();
  }
  ...
  protected void create() {
    ...
    HyperboloidOfOneSheetBuilder builder = new HyperboloidOfOneSheetBuilder(hyperboloidParams);
  ...
  protected String description() {
    return "<html><strong>Однополостный гиперболоид</strong><br>по трем точкам";
  }
