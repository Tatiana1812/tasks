 ...
 * @author Vitaliy
 */
public class LineXEllipsoidBuilder extends BodyBuilder {
  
  static public final String ALIAS = "LineXEllipsoid";

  public LineXEllipsoidBuilder() {
  }

  public LineXEllipsoidBuilder(String id, String name) {
    super(id, name);
  }

  public LineXEllipsoidBuilder(String id, HashMap<String, BuilderParam> params) {
    super(id, params);
  }

  public LineXEllipsoidBuilder(String id, String name, JsonObject param) {
    super(id, name);
    setValue(BLDKEY_LINE, param.get(BLDKEY_LINE).asString());
    setValue(BLDKEY_ELLIPSOID, param.get(BLDKEY_ELLIPSOID).asString());
  }

  public LineXEllipsoidBuilder(HashMap<String, BuilderParam> params) {
    super(params);
  }
  ...
  public i_Body create(Editor edt, i_ErrorHandler eh) {
    ...
    try {
      ...
      intersectPoints = el.ellipsoid().intersect(l.line());
      i_Body result;
      ...
      if (_exists) {
        eh.showMessage("Пересечение прямой и эллипсоида не построено: " + ex.getMessage(), error.Error.WARNING);
      }
      _exists = false;
  ...
  public String description(EdtController ctrl, int precision) {
    try {
      return String.format("<html><strong>Сечение эллипсоида %s</strong><br> прямой %s",
              ctrl.getBodyTitle(getValueAsString(BLDKEY_ELLIPSOID)),
              ctrl.getBodyTitle(getValueAsString(BLDKEY_LINE)));
    } catch (ExNoBody ex) {
      return "<html><strong>Сечение эллипсоида прямой</strong>";
    }
  }
