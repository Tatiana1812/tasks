...
/**
 * Constructs a section of prism
 *
 * @author alexeev
 */
public class PrismSectionBuilder extends BodyBuilder {
  static public final String ALIAS = "PrismSection";

  public PrismSectionBuilder() {
  }

  public PrismSectionBuilder(String id, String name) {
    super(id, name);
  }

  public PrismSectionBuilder(String id, HashMap<String, BuilderParam> params) {
    super(id, params);
  }

  public PrismSectionBuilder(String id, String name, JsonObject param) {
    super(id, name);
    setValue(BLDKEY_PRISM, param.get(BLDKEY_PRISM).asString());
    setValue(BLDKEY_PLANE, param.get(BLDKEY_PLANE).asString());
  }

  public PrismSectionBuilder(HashMap<String, BuilderParam> params) {
    super(params);
  }
  ...
  public void initParams(){
    super.initParams();
    addParam(BLDKEY_PRISM, BuilderParam.PRISM_ALIAS, BuilderParamType.BODY, 61);
  ...
  public i_Body create(Editor edt, i_ErrorHandler eh) {
    ...
    try {
      ...
      Polygon3d poly = prism.prism().sectionByPlane(p.plane());
      ...
      PolygonBody result = prism.section(_id, p);
      ...
      if (_exists) {
        eh.showMessage("Сечение призмы не построено: " + ex.getMessage(), error.Error.WARNING);
      }
      _exists = false;
  ...
  public String description(EdtController ctrl, int precision) {
    try {
      return String.format("<html><strong>Сечение</strong> призмы %s плоскостью %s",
              ctrl.getBodyTitle(getValueAsString(BLDKEY_PRISM)),
              ctrl.getBodyTitle(getValueAsString(BLDKEY_PLANE)));
    } catch (ExNoBody ex) {
      return "<html><strong>Сечение призмы</strong>";
    }
  }
}
