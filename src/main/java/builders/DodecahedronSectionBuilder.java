...
/**
 * Constructs section of the dodecahedron
 *
 * @author Vladislav
 */
public class DodecahedronSectionBuilder extends BodyBuilder {
  static public final String ALIAS = "DodecahedronSection";
  
  public DodecahedronSectionBuilder() {
  }

  public DodecahedronSectionBuilder(String id, String name) {
    super(id, name);
  }

  public DodecahedronSectionBuilder(String id, HashMap<String, BuilderParam> params) {
    super(id, params);
  }

  public DodecahedronSectionBuilder(String id, String name, JsonObject param) {
    super(id, name);
    setValue(BLDKEY_PLANE, param.get(BLDKEY_PLANE).asString());
    setValue(BLDKEY_DODECAHEDRON, param.get(BLDKEY_DODECAHEDRON).asString());
  }

  public DodecahedronSectionBuilder(HashMap<String, BuilderParam> params) {
    super(params);
  }
  ...
  public void initParams(){
    super.initParams();
    addParam(BLDKEY_DODECAHEDRON, "Додекаэдр", BuilderParamType.BODY, 61);
  ...
  public i_Body create(Editor edt, i_ErrorHandler eh) {
    ...
    try {
      ...
      DodecahedronBody o = (DodecahedronBody) bodies.get(getValueAsString(BLDKEY_DODECAHEDRON));
      if (!o.exists()) {
        throw new ExLostBody("додекаэдр");
      }

      Polygon3d poly = o.dodecahedron().sectionByPlane(p.plane());
      ...
      if (_exists) {
        eh.showMessage("Сечение додекаэдра не построено: " + ex.getMessage(), error.Error.WARNING);
      }
      _exists = false;
  ...
  public String description(EdtController ctrl, int precision) {
    try {
      return String.format("<html><strong>Сечение</strong> додекаэдра %s плоскостью %s",
              ctrl.getBodyTitle(getValueAsString(BLDKEY_DODECAHEDRON)),
              ctrl.getBodyTitle(getValueAsString(BLDKEY_PLANE)));
    } catch (ExNoBody ex) {
      return "<html><strong>Сечение додекаэдра</strong>";
    }
  }
}
