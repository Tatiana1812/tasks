...
/**
 * Constructs section of the tetrahedron
 *
 * @author Vladislav
 */
public class TetrahedronSectionBuilder extends BodyBuilder {
  static public final String ALIAS = "TetrahedronSection";

  public TetrahedronSectionBuilder() {
  }

  public TetrahedronSectionBuilder(String id, String name) {
    super(id, name);
  }

  public TetrahedronSectionBuilder(String id, HashMap<String, BuilderParam> params) {
    super(id, params);
  }

  public TetrahedronSectionBuilder(String id, String name, JsonObject param) {
    super(id, name);
    setValue(BLDKEY_TETRAHEDRON, param.get(BLDKEY_TETRAHEDRON).asString());
    setValue(BLDKEY_PLANE, param.get(BLDKEY_PLANE).asString());
  }

  public TetrahedronSectionBuilder(HashMap<String, BuilderParam> params) {
    super(params);
  }
  ...
  public void initParams(){
    super.initParams();
    addParam(BLDKEY_TETRAHEDRON, BuilderParam.TETRAHEDRON_ALIAS, BuilderParamType.BODY, 61);
  ...
  public i_Body create(Editor edt, i_ErrorHandler eh) {
    ...
    try {
      ...
      TetrahedronBody t = (TetrahedronBody) bodies.get(getValueAsString(BLDKEY_TETRAHEDRON));
      if (!t.exists()) {
        throw new ExLostBody("тетраэдр");
      }

      Polygon3d poly = t.tetrahedron().sectionByPlane(p.plane());
      ...
      PolygonBody result = t.section(_id, p);
      ...
      if (_exists) {
        eh.showMessage("Сечение тетраэдра не построено: " + ex.getMessage(), error.Error.WARNING);
      }
      _exists = false;
  ...
  public String description(EdtController ctrl, int precision) {
    try {
      return String.format("<html><strong>Сечение</strong> тетраэдра %s плоскостью %s",
              ctrl.getBodyTitle(getValueAsString(BLDKEY_TETRAHEDRON)),
              ctrl.getBodyTitle(getValueAsString(BLDKEY_PLANE)));
    } catch (ExNoBody ex) {
      return "<html><strong>Сечение тетраэдра</strong>";
    }
  }
}
